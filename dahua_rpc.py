...
class DahuaRpc(object):
    ...
    def keep_alive(self):
        params = {"timeout": 300, "active": False}
    ...
    def get_traffic_info(self):
        ...
        params = {"name": "TrafficSnapEventInfo"}
        r = self.request(method=method, params=params)
    ...
    def start_find(self, object_id):
        ...
        object_id = object_id
        params = {"condition": {"Time": ["<>", 1558925818, 1559012218]}}
    ...
    def listen_events(self, _callback=None):
        ...
        url = "http://{host}/SubscribeNotify.cgi?sessionId={session}".format(
            host=self.host, session=self.session_id
        )
        response = self.s.get(url, stream=True)

        buffer = ""
        for chunk in response.iter_content(chunk_size=1):
            buffer += chunk.decode("latin-1").encode("utf-8").decode("utf-8")
